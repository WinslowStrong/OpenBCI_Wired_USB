...
void OpenBCI_32_Daisy::sendChannelData(){
  Serial1.write(0xA0);  //WS manually prepend the packet header (Rfduino did it before)
  ...
  Serial1.write(0xC0);       //WS manually append the packet footer (Rfduino did it before)
  sampleCounter++;
}

void OpenBCI_32_Daisy::writeAuxData(){
  for(int i=0; i<3; i++){
    Serial1.write(highByte(auxData[i])); // write 16 bit axis data MSB first
    Serial1.write(lowByte(auxData[i]));  // axisData is array of type short (16bit)
...
// initalize the  data ready chip select and reset pins:
    ...
    delay(10);
    WREG(CONFIG1,0xB0+Rate_Adj,BOARD_ADS); //WS tell on-board ADS to output its clk, set the data rate to 500SPS
    delay(40);
...
boolean OpenBCI_32_Daisy::smellDaisy(void){ // check if daisy present
  ...
  if(verbosity){Serial1.print("Daisy ID 0x"); Serial1.println(setting,HEX);}
...
void OpenBCI_32_Daisy::removeDaisy(void){
  if(daisyPresent){
    ...
    STANDBY(DAISY_ADS);
    daisyPresent = false;
    if(!isRunning) Serial1.println("daisy removed");
  }else{
    if(!isRunning) Serial1.println("no daisy to remove!");
  }
}

void OpenBCI_32_Daisy::attachDaisy(void){
  WREG(CONFIG1,0xB0+Rate_Adj,BOARD_ADS); //WS tell on-board ADS to output the clk, set the data rate to 250SPS
  delay(40);
  resetADS(DAISY_ADS); // software reset daisy module if present
  delay(10);
  daisyPresent = smellDaisy(); 
  if(!daisyPresent){
    WREG(CONFIG1,0x90+Rate_Adj,BOARD_ADS); //WS turn off clk output if no daisy present
    numChannels = 8;    // expect up to 8 ADS channels
    if(!isRunning) Serial1.println("no daisy to attach!");
  }else{
    numChannels = 16;   // expect up to 16 ADS channels
    if(!isRunning) Serial1.println("daisy attached");
  } 
}
...
void OpenBCI_32_Daisy::reportDefaultChannelSettings(void){
    ...
    Serial1.write(defaultChannelSettings[POWER_DOWN] + '0');        // on = NO, off = YES
    Serial1.write((defaultChannelSettings[GAIN_SET] >> 4) + '0');     // Gain setting
    Serial1.write(defaultChannelSettings[INPUT_TYPE_SET] +'0');// input muxer setting
    Serial1.write(defaultChannelSettings[BIAS_SET] + '0');    // add this channel to bias generation
    Serial1.write(defaultChannelSettings[SRB2_SET] + '0');       // connect this P side to SRB2
    Serial1.write(defaultChannelSettings[SRB1_SET] + '0');        // don't use SRB1
}
...
void OpenBCI_32_Daisy::changeChannelLeadOffDetect(byte N) // N arrives as zero indexed
{
  ...
  if(N < 0x08){
    ...
    if(!daisyPresent) { Serial1.println("no daisy attached!"); return; }
...
{ 
  ...
  if(daisyPresent){
    if(sampleCounter % 2 != 0){ //CHECK SAMPLE ODD-EVEN AND SEND THE APPROPRIATE ADS DATA
      for (int i=0; i<24; i++){ 
//        Serial1.write(meanBoardDataRaw[i]); // send board data on odd samples
          Serial1.write(boardChannelDataRaw[i]); //WS - no need to take mean
      }
    }else{
      for (int i=0; i<24; i++){
//        Serial1.write(meanDaisyDataRaw[i]); // send daisy data on even samples
           Serial1.write(daisyChannelDataRaw[i]); //WS - no need to take mean
      }
    }
  }else{
    for(int i=0; i<24; i++){
...
byte OpenBCI_32_Daisy::ADS_getDeviceID(int targetSS) {      // simple hello world com check
  ...
  if(verbosity){            // verbosity otuput
    Serial1.print("On Board ADS ID ");
    printHex(data); Serial1.println();
  }
  return data;
}
...
byte OpenBCI_32_Daisy::RREG(byte _address,int targetSS) {    //  reads ONE register at _address
  ...
  if (verbosity){           //  verbosity output
    ...
    Serial1.print(", ");
    for(byte j = 0; j<8; j++){
      Serial1.print(bitRead(regData[_address], 7-j));
      if(j!=7) Serial1.print(", ");
    }
...
void OpenBCI_32_Daisy::RREGS(byte _address, byte _numRegistersMinusOne, int targetSS) {
  ...
  if(verbosity){            //  verbosity output
    for(int i = 0; i<= _numRegistersMinusOne; i++){
      ...
      Serial1.print(", ");
      for(int j = 0; j<8; j++){
        Serial1.print(bitRead(regData[_address + i], 7-j));
        if(j!=7) Serial1.print(", ");
      }
      Serial1.println();
      delay(30);
...
void OpenBCI_32_Daisy::WREGS(byte _address, byte _numRegistersMinusOne, int targetSS) {
  ...
  for (int i=_address; i <=(_address + _numRegistersMinusOne); i++){
    ...
    csHigh(targetSS);
  if(verbosity){
    Serial1.print("Registers ");
    printHex(_address); Serial1.print(" to ");
...
void OpenBCI_32_Daisy::LIS3DH_writeAxisData(void){
  for(int i=0; i<3; i++){
    Serial1.write(highByte(axisData[i])); // write 16 bit axis data MSB first
    Serial1.write(lowByte(axisData[i]));  // axisData is array of type short (16bit)
    axisData[i] = 0;
  }
}
...
void OpenBCI_32_Daisy::LIS3DH_readAllRegs(){
  ...
  for (int i = STATUS_REG_AUX; i <= WHO_AM_I; i++){
    ...
    Serial1.print("0x0");Serial1.print(i,HEX);
    Serial1.print("\t");Serial1.println(inByte,HEX);
    delay(20);
  }
  Serial1.println();
  ...
  for (int i = TMP_CFG_REG; i <= INT1_DURATION; i++){
    inByte = LIS3DH_read(i);
    // printRegisterName(i);
    Serial1.print("0x");Serial1.print(i,HEX);
    Serial1.print("\t");Serial1.println(inByte,HEX);
    delay(20);
  }
  Serial1.println();
...
void OpenBCI_32_Daisy::printRegisterName(byte _address) {
    switch(_address){ 
      case ID_REG: 
        Serial1.print("ADS_ID, "); break;
      case CONFIG1:
        Serial1.print("CONFIG1, "); break;
      case CONFIG2:
        Serial1.print("CONFIG2, "); break;
      case CONFIG3:
        Serial1.print("CONFIG3, "); break;
      case LOFF:
        Serial1.print("LOFF, "); break;
      case CH1SET:
        Serial1.print("CH1SET, "); break;
      case CH2SET:
        Serial1.print("CH2SET, "); break;
      case CH3SET:
        Serial1.print("CH3SET, "); break;
      case CH4SET:
        Serial1.print("CH4SET, "); break;
      case CH5SET:
        Serial1.print("CH5SET, "); break;
      case CH6SET:
        Serial1.print("CH6SET, "); break;
      case CH7SET:
        Serial1.print("CH7SET, "); break;
      case CH8SET:
        Serial1.print("CH8SET, "); break;
      case BIAS_SENSP:
        Serial1.print("BIAS_SENSP, "); break;
      case BIAS_SENSN:
        Serial1.print("BIAS_SENSN, "); break;
      case LOFF_SENSP:
        Serial1.print("LOFF_SENSP, "); break;
      case LOFF_SENSN:
        Serial1.print("LOFF_SENSN, "); break;
      case LOFF_FLIP:
        Serial1.print("LOFF_FLIP, "); break;
      case LOFF_STATP:
        Serial1.print("LOFF_STATP, "); break;
      case LOFF_STATN:
        Serial1.print("LOFF_STATN, "); break;
      case GPIO:
        Serial1.print("GPIO, "); break;
      case MISC1:
        Serial1.print("MISC1, "); break;
      case MISC2:
        Serial1.print("MISC2, "); break;
      case CONFIG4:
        Serial1.print("CONFIG4, "); break;
      default: 
        break;
    }
}
...
void OpenBCI_32_Daisy::printHex(byte _data){
  Serial1.print("0x");
    if(_data < 0x10) Serial1.print("0");
    Serial1.print(_data, HEX);
}
