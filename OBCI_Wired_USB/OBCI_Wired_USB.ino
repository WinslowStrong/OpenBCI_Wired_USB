...
/*
 *  WS - This version is based off of the SD_Only version, replacing all Serial0 communication with Serial1, whose baud rate can be set.
 *  WS - It must be accompanied by the library files with the same change made.
 *  WS - Added code to allow changing the sample rate via the Rate_Adj variable.
 *  WS - Changed the way that daisy mode sends data - instead of sending ch. 1-8, waiting 1/SampleRate, sending ch. 9-16, etc, it sends Ch 1-8 and immediatly sends ch9-16 in the next packet.
 *
 ...
 *
 *      This version, called OpenBCI_32_SD_Only has an added feature to optionally log
 *      data to the SD card only. To do this, send an 'N' over the serial port.
 *
 *
...
//  Serial1.begin(460800);  //Uses UART1.  
  Serial1.begin(230400);  //Uses UART1.  
//  Serial1.begin(115200);  //Uses UART1.  
  ...
  //  Serial0.begin(115200);  //WS The default way to send over the Rfduino
  //  mapPps(11,PPS_OUT_U2TX); //WS seems to be obsolete
  //  mapPps(12,PPS_IN_U2RX);  //WS seems to be obsolete
  ...
  if (is_running) {
    ...
    if (!writeToSDonly) {
      ...
      if (OBCI.daisyPresent) {
        OBCI.sendChannelData(); //WS-daisy: sends the rest of the channels, using the same packet formatting.
      }
      if (SDfileOpen) {
      ...
      if (SDfileOpen) {
        ...
        if (SDsampleCounter == 0x00) {
          Serial1.print("SD BLOCKS written: ");
          Serial1.println(blockCounter); // verbosity
        }
      }
    }
...
void eventSerial() {
  while (Serial1.available()) {
    ...
    //WS this is in lieu of the burger protocol
    testChar = inChar;
    ...
    //WS - below is the burger protocol way of doing it
...
void getCommand(char token) {
  switch (token) {
    ...
    case 'z':  // expect 2 parameters
      if (!is_running) {
        Serial1.println("ready to accept new impedance detect settings");
      }
    ...
    case 'Z':  // latch impedance parameters
      if (!is_running) {
        Serial1.println("updating impedance detect settings");
      }
    ...
    case 'C':  // use 16 channel mode
      ...
      if (OBCI.daisyPresent) {
        Serial1.print("16"); outputType = OUTPUT_16_CHAN;
      } else {
        Serial1.print("8"); outputType = OUTPUT_8_CHAN;
      }
      sendEOT();
      break;
...
void sendEOT() {
  Serial1.println("$$$");  // shake hands with the controlling program
}
...
void loadChannelSettings(char c) {
  ...
  if (channelSettingsCounter == 0) { // if it's the first byte in this channel's array, this byte is the channel number to set
    ...
    if (!is_running) {
      Serial1.print("load setting ");
      Serial1.print("for channel ");
      Serial1.println(currentChannelToSet + 1, DEC);
    }
    return;
  }
  //  setting bytes are in order: POWER_DOWN, GAIN_SET, INPUT_TYPE_SET, BIAS_SET, SRB2_SET, SRB1_SET
  if (!is_running) {
    Serial1.print(channelSettingsCounter - 1);
    Serial1.print(" with "); Serial1.println(c);
  }
  c -= '0';
  ...
  if (channelSettingsCounter == 7) { // 1 currentChannelToSet, plus 6 channelSetting parameters
    if (!is_running) Serial1.print("done receiving settings for channel "); Serial1.println(currentChannelToSet + 1, DEC);
...
void loadLeadOffSettings(char c) {
  if (leadOffSettingsCounter == 0) { // if it's the first byte in this channel's array, this byte is the channel number to set
    currentChannelToSet = getChannelNumber(c); // we just got the channel to load settings into (shift number down for array usage)
    if (!is_running) Serial1.print("changing LeadOff settings for channel "); Serial1.println(currentChannelToSet + 1, DEC);
    leadOffSettingsCounter++;
    return;
  }
  //  setting bytes are in order: PCHAN, NCHAN
  if (!is_running) {
    Serial1.print("load setting "); Serial1.print(leadOffSettingsCounter - 1);
    Serial1.print(" with "); Serial1.println(c);
  }
  c -= '0';
  ...
  if (leadOffSettingsCounter == 3) { // 1 currentChannelToSet, plus 2 leadOff setting parameters
    if (!is_running) Serial1.print("done receiving leadOff settings for channel "); Serial1.println(currentChannelToSet + 1, DEC);
...
void startFromScratch() {
  if (!is_running) {
    ...
    OBCI.Rate_Adj = Rate_Adj; //WS  Set the Sample rate adjustor
    delay(500);
    Serial1.println("OpenBCI V3 16 channel");
    Serial1.print  ("SD Only Sample Rate : "           )   ;   Serial1.println( 250 << (0b110 - Rate_Adj)  );  delay(10);
    OBCI.configureLeadOffDetection(LOFF_MAG_6NA, LOFF_FREQ_31p2HZ);
    Serial1.print("On Board ADS1299 Device ID: 0x"); Serial1.println(OBCI.ADS_getDeviceID(ON_BOARD), HEX);
    if (OBCI.daisyPresent) { // library will set this in initialize() if daisy present and functional
      Serial1.print("On Daisy ADS1299 Device ID: 0x"); Serial1.println(OBCI.ADS_getDeviceID(ON_DAISY), HEX);
    }
    Serial1.print("LIS3DH Device ID: 0x"); Serial1.println(OBCI.LIS3DH_getDeviceID(), HEX);
    sendEOT();
  }
}
