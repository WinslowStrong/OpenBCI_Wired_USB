...
boolean setupSDcard(char limit){
  ...
  if(!cardInit){
      ...
      Serial1.println("Could not find FAT16/FAT32 partition. Make sure you've formatted the card");
      return fileIsOpen;
      }
   }
  ...
  default: Serial1.println("invalid BLOCK count"); return fileIsOpen; break;
  }
    ...
    openfile.remove(root, currentFileName); // if the file is over-writing, let it!
      ...
      Serial1.print("createfdContiguous fail"); cardInit = false;
      }//else{Serial1.print("got contiguous file...");delay(1);}
      // get the location of the file's blocks
      if (!openfile.contiguousRange(&bgnBlock, &endBlock)) {
      Serial1.print("get contiguousRange fail"); cardInit = false;
      }//else{Serial1.print("got file range...");delay(1);}
      // grab the Cache
      pCache = (uint8_t*)volume.cacheClear();
      // tell card to setup for multiple block write with pre-erase
      if (!card.erase(bgnBlock, endBlock)){ Serial1.println("erase block fail"); cardInit = false;
      }//else{Serial1.print("erased...");delay(1);}
      if (!card.writeStart(bgnBlock, BLOCK_COUNT)){ Serial1.println("writeStart fail"); cardInit = false;
     ...
     if(fileIsOpen == true){  // send corresponding file name to controlling program
       Serial1.print("Corresponding SD file ");Serial1.println(currentFileName);sendEOT();
     }
     return fileIsOpen;
}
...
boolean closeSDfile(){
  if(fileIsOpen){
    ...
    if(!is_running){ // verbosity. this also gets insterted as footer in openFile
      Serial1.print("Total Elapsed Time: ");Serial1.print(t);Serial1.println("mS"); delay(10);
      Serial1.print("Max write time: "); Serial1.print(maxWriteTime); Serial1.println(" uS"); delay(10);
      Serial1.print("Min write time: ");Serial1.print(minWriteTime); Serial1.println(" uS"); delay(10);
      Serial1.print("Overruns: "); Serial1.print(overruns); Serial1.println(); delay(10);
      if (overruns) {
        uint8_t n = overruns > OVER_DIM ? OVER_DIM : overruns;
        Serial1.println("fileBlock,micros");
        for (uint8_t i = 0; i < n; i++) {
          Serial1.print(over[i].block); Serial1.print(','); Serial1.println(over[i].micro);
        }
      }
    }
  }else{
    Serial1.println("no open file to close");
  }
  delay(100); // cool down
  return fileIsOpen;
}
...
void writeCache(){
    ...
    OBCI.csLow(SD_SS);  // take spi
    if(!card.writeData(pCache)) {Serial1.println("block write fail");}   // write the block
...
//  Serial1.print("Corresponding SD file ");Serial1.println(currentFileName);
}
